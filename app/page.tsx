...
export default function Home() {
  ...
  const handlePreview = () => {
    try {
      ...
      // Check if any rule is missing the path field
      const hasRuleWithoutPath = rules.some(rule => !rule.path || rule.path.trim() === '');
      
      if (hasRuleWithoutPath) {
        // Store entire array as "policy"
        setPreview([{
          key: 'policy',
          value: rules as any,
          path: 'Full Policy Array'
        }]);
        setMessage({ 
          type: 'success', 
          text: `Preview generated: Storing entire policy array (${rules.length} rules) with key "policy"` 
        });
        return;
      }

      const previewData: Array<{ key: string; value: RedirectRule; path: string }> = [];
      
      rules.forEach(rule => {
        // Check if path starts with '!' (negation) - remove it for processing
        let pathString = rule.path.trim();
        const isNegation = pathString.startsWith('!');
        if (isNegation) {
          pathString = pathString.substring(1).trim();
        }
        
        // Split paths by space and filter out empty strings
        const paths = pathString.split(/\s+/).filter(Boolean);
        
        // Create a key-value pair for each path
        paths.forEach(singlePath => {
          previewData.push({
            key: hexEncode(singlePath),
            value: {
              ...rule,
              path: singlePath // Store individual path in the rule
            },
            path: singlePath
          });
        });
      ...
      setMessage({ type: 'success', text: `Preview generated for ${previewData.length} entries from ${rules.length} rules` });
    } catch (error) {
      ...
      const response = await fetch('/kv-manager', {
        ...
        body: JSON.stringify({
          ...
          entries: preview.map(p => ({
            key: p.key,
            value: p.key === 'policy' ? JSON.stringify(p.value) : JSON.stringify(p.value)
          }))
        }),
      });
